...
export default {
  ...
  computed: {
    ...
    getPluginData() {
      return this.$store.getters['getActivePlugin'] && this.$store.getters['getActivePlugin'].map(val => Object.fromEntries(Object.entries(val).filter(([key]) => !key.includes('TMP_topic') && !key.includes('TMP_type') && !key.includes('old_values'))))
    },
    getDownloadLink() {
